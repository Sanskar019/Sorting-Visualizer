...
class sortAlgorithms {
    ...
    SelectionSort = async () => {
        for(let i = 0 ; i < this.size ; ++i) {
            ...
            for(let j = i ; j < this.size ; ++j) {
                await this.help.markIndex(minIndex);
                await this.help.mark(j);
    ...
    // MERGE SORT
    MergeSort = async () => {
        await this.MergeArray(0, this.size - 1);
        for(let counter = 0 ; counter < this.size ; ++counter) {
            this.list[counter].setAttribute("class", "cell done");
        }
    }

    MergeArray = async (start, end) => {
        if(start < end) {
            let mid = start + Math.floor((end - start)/2);
            await this.MergeArray(start, mid);
            await this.MergeArray(mid+1, end);
    ...
    // QUICK SORT
    QuickSort = async () => {
        await this.QuickDivide(0, this.size-1);
        for(let c = 0 ; c < this.size ; ++c) {
            this.list[c].setAttribute("class", "cell done");
        }
    }

    QuickDivide = async (start, end) => {
        if(start < end) {
            let pivot = await this.Partition(start, end);
            await this.QuickDivide(start, pivot-1);
            await this.QuickDivide(pivot+1, end);
        }
    }
